...
事实上， [Angular 2](https://angular.io)使用TypeScript 增加了更多的牵引
这儿有关于在js中使用此类型的利弊，阅读这两篇文章，提出你自己的观点。
...
基本流程内使用的代码中添加注释来描述装置类型（阅读这里更多关于使用Flow写模块）
...
它使用Facebook内部的源代码项目，（React，React Native，Flux，Immutable，Jest等）Facebook成为开源世界的主演。
...
[Gulp](http://gulpjs.com) Gulp是一个通用的任务运行器，可以用于涉及文件系统的任何类型的自动过程，因此它不是Webpack或Browserify的直接竞争者。

类似于 [Grunt](http://gruntjs.com/),Gulp通过聚合工作：你可以要求它缩小和连接资源列表，但它不会像Webpack或Browserify那样自己处理模块化JavaScript。
...
基本上，它需要几个node.js包作为输入，并为浏览器生成一个单一的“构建”文件作为输出。
 但是它似乎是一个像Webpack一样的工具，更适合Web应用工作流。
...
最有名的两个测试框架是 [Jasmine](http://jasmine.github.io/) 和 [Mocha](https://mochajs.org)， 但最近两个项目在2016年有更多的牵引力: [AVA](https://github.com/avajs/ava) 和 [Jest](http://facebook.github.io/jest/).
...
关于 IDE (_Integrated Development Environment_),值得一提的是，最流行的两个IDE是使用Web技术开发的开源项目。 
...
一些开发人员提到关于开发速度感谢IntelliSense功能的改进(和自动完成).
